...
/// Round 5: every test in this file runs in one `.serialized` suite rather
/// than as top-level free functions. `concurrentStartAndDeliveryGenerationChecksDoNotRace`
/// deliberately saturates the process for a fixed 3-second window (that's
/// what makes its own race reproducible under TSan — see its own doc
/// comment), and under Swift Testing's default parallel execution that
/// window could delay a *different* test's real filesystem event past its
/// budget: `stopDuringAPendingEmptinessSettleDoesNotResurrectTheWatcher`
/// measurably timed out under that contention even at a widened 15s (round
/// 4). Widening further would be the same mistake this project has already
/// paid for three times elsewhere (the growth floor, the stable-file bound,
/// the reading-claim ceiling) — a number chosen to outrun a problem instead
/// of removing it. `.serialized` removes it instead: no two tests in this
/// suite run concurrently with each other, so the saturating test has the
/// machine to itself, and every other test's timeouts can go back to being
/// honest rather than defensive. `.serialized` only affects Swift Testing's
/// own scheduling between tests — it does not touch concurrency a test
/// creates *within itself* (verified: the race test's own `withTaskGroup`
/// still exercises the guard it needs to, 5/5 TSan warnings against its
/// mutation, unchanged by this wrapping — see its doc comment).
@Suite(.serialized)
struct FolderWatcherTests {
    ...
    @Test func droppingTheWatcherWithoutStoppingClosesItsDescriptor() async throws {
        ...
        // test (now the whole suite's default — see the type doc) removes that
        // specific interference for tests in *this* file, but Swift Testing
        // still parallelizes other files/targets in the same process, so the
        // identity check below remains the right fix regardless — checking the
        // descriptor's *identity* — what path it actually points at, via
        // F_GETPATH — is immune to reuse: a closed descriptor fails outright,
        // and a reused one resolves to a different path.
        let temp = try TempDirectory()
    ...
    @Test func concurrentStartStopAndReadsDoNotCorruptState() async throws {
        ...
        // The concurrency here is internal to the test (its own withTaskGroup),
        // unaffected by this suite's `.serialized` trait — see the type doc.
        let temp = try TempDirectory()
        let watcher = FolderWatcher(folders: [temp.url]) { _ in }
    ...
    @Test func stopDuringAPendingEmptinessSettleDoesNotResurrectTheWatcher() async throws {
        ...
        // state transitions rather than guessed timing:
        //
        ...
        //
        // Round 4 widened these waits to 15s / 1500ms to absorb interference
        // from `concurrentStartAndDeliveryGenerationChecksDoNotRace` running
        // concurrently elsewhere in this file — and that widening still left a
        // measured ~2% residual timeout rate (1 failure in 51 runs), because
        // absorbing contention with a bigger number doesn't remove it. Round 5
        // removes the contention instead: this whole file now runs as one
        // `.serialized` suite (see the type doc), so nothing else in it can be
        // running while this test's real filesystem events need `queue`'s
        // attention. Timeouts below are back to the original, honest budget.
        let temp = try TempDirectory()
        try temp.writeFile("pre-existing.png")
    ...
    @Test func concurrentStartAndDeliveryGenerationChecksDoNotRace() async throws {
        ...
        //
        // Round 5: this file's `.serialized` suite trait (see the type doc)
        // stops this test from running *alongside* other tests, but does not
        // touch the concurrency this test creates *within itself* below —
        // `.serialized` only orders Swift Testing's own test-to-test
        // scheduling. Reconfirmed after adding the trait: still 5/5 TSan
        // warnings against the mutation, still 2/2 clean unmutated (see the
        // report) — the internal race this test depends on is untouched.
        let temp = try TempDirectory()
